...
function PopupQuiz({ ...props }) {
  const { isPopupQuizOpen, setIsPopupQuizOpen } = useStorage();

  return (
    <>
      <Modal className="rules-modal custom-modal" open={isPopupQuizOpen} closeIcon={null} footer={null} centered>
        <div className="rules-modal-content space-y-3">
          ...
          <button className="secondary-btn flex w-full" onClick={() => setIsPopupQuizOpen(false)}>
            Đóng
          </button>
        </div>
      </Modal>
